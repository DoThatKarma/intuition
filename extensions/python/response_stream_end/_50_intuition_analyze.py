...
class IntuitionAnalyzeEnd(Extension):
    """Complete mode / fallback: start analysis when the response stream ends."""

    async def execute(self, loop_data=LoopData(), **kwargs):
        if not self.agent or not _helper_ok():
            return
        try:
            _helper.get_watcher(self.agent).start_analysis(self.agent)
        except Exception:
            pass
