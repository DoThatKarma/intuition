...
class IntuitionDeliverLate(Extension):
    """Late-delivery sweep before the next model call.

    Delivers hints from the previous iteration's watcher and from any parked
    watchers whose background analyses finished in the meantime.
    """
    ...
    async def execute(self, loop_data=LoopData(), **kwargs):
        if not self.agent or not _helper_ok():
            return
        try:
            await _helper.deliver_pending(self.agent)
        except Exception:
            pass
